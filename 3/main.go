...
func first(w1, w2 []string) {
	...
	for x1, v1 := range m1 {
		for y1 := range v1 {
			...
			if x2, exists := m2[x1]; exists {
				if _, exists := x2[y1]; exists {
...
func fillMem(w []string) map[int]map[int]int {
	cx := 0
	cy := 0
	...
	for _, d := range w {
		...
		case 'L':
			for val > 0 {
				cx--
				if _, exists := mem[cx]; !exists {
					mem[cx] = map[int]int{}
				}
				if _, exists := mem[cx][cy]; !exists {
					mem[cx][cy] = steps
				}
				val--
				steps++
